...
const FaturamentoTable = ({ faturamentos }) => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'A':
      case 'N': // NF Normal/Ativa
        return <span className="status-badge status-a">Ativa</span>;
      default:
  ...
  if (!faturamentos || faturamentos.length === 0) {
    return <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-muted)' }}>Nenhum faturamento encontrado.</div>;
  }
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
          <thead>
            <tr>
              ...
              <th style={thStyle(widths.vlrMerc)} onClick={() => requestSort('NOT_RE_VALORMERCADORIA')}>
                Vlr Mercadoria{getSortIcon('NOT_RE_VALORMERCADORIA')}
                <div onMouseDown={(e) => onMouseDown(e, 'vlrMerc')} style={resizerStyle} />
              </th>
          ...
          <tbody>
            {sortedData.map((fat, index) => (
              ...
              >
                ...
                <td style={tdStyle(widths.vlrMerc)}>{formatCurrency(fat.NOT_RE_VALORMERCADORIA)}</td>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              ...
              <th style={{ padding: '8px', border: '1px solid #ddd', fontSize: '8pt' }}>VLR MERC.</th>
              <th style={{ padding: '8px', border: '1px solid #ddd', fontSize: '8pt' }}>VLR TOTAL</th>
            </tr>
          </thead>
          <tbody>
            {faturamentos.map((fat) => (
              <tr key={`print-${fat.NOT_IN_NUMERO}`}>
                ...
                <td style={{ padding: '6px', border: '1px solid #ddd', fontSize: '7pt', textAlign: 'center' }}>{fat.PEDIDOS_DATAS || '-'}</td>
                ...
                <td style={{ padding: '6px', border: '1px solid #ddd', fontSize: '7pt', textAlign: 'center' }}>{formatCurrency(fat.NOT_RE_VALORMERCADORIA)}</td>
